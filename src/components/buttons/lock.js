  ...
  async execute(interaction, client) {
    ...
    exe.execute(async () => {
      ...
      if (!permissions.has(PermissionFlagsBits.Connect)) {
        await embeds.room.AlreadyAction(interaction,"Room is Already locked Mr.",`<@${member.id}>`,true);
        setTimeout(() => {
        ...
        // locked message
        await embeds.room.successfulAction(interaction,"locked"," the voice channel",true);
        setTimeout(() => {
          interaction.deleteReply();
        },5000)
      }
