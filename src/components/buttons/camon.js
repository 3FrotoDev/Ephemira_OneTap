    ...
    async execute(interaction, client) {
      ...
      exe.execute(async () => {
        ...
        if (permissions.has(PermissionFlagsBits.Stream)) {
          await embeds.room.AlreadyAction(interaction,"Stream is Already Enabled","...",true);
          setTimeout(() => {
          ...
          // locked message
          await embeds.room.successfulAction(interaction,"Enabled"," Cam & Stream for all members",true);
          setTimeout(() => {
            interaction.deleteReply();
          },5000)
        }
